...
class TemplateExtractor {
    constructor(options = {}) {
        ...
        this.options = {
            headless: options.headless !== undefined ? options.headless : this.config.extractor.headless,
            timeout: options.timeout || this.config.extractor.timeout,
            ...
            screenshotEnabled: options.screenshotEnabled !== undefined ? options.screenshotEnabled : this.config.extractor.screenshot.enabled,
            screenshotFullPage: options.screenshotFullPage !== undefined ? options.screenshotFullPage : this.config.extractor.screenshot.fullPage,
            ...options
        };
    ...
    loadConfig() {
        try {
            ...
            console.warn('⚠️ نتوانست config.json را بارگذاری کند، استفاده از تنظیمات پیش‌فرض');
            return {
                extractor: {
                    headless: true,
                    timeout: 30000,
    ...
    async extractTemplate(url, outputDir) {
        ...
        try {
            ...
            const browserOptions = {
                headless: this.config.extractor.headless,
                args: [
                    '--no-sandbox', 
            ...
            await page.goto(url, { 
                ...
                timeout: this.config.extractor.timeout 
            });
            ...
            // گرفتن screenshot
            if (this.config.extractor.screenshot.enabled) {
                this.addLog('info', 'گرفتن screenshot...');
                try {
                    const screenshotPath = path.join(outputDir, `screenshot.${this.config.extractor.screenshot.format}`);
                    await page.screenshot({
                        path: screenshotPath,
                        fullPage: this.config.extractor.screenshot.fullPage,
                        type: this.config.extractor.screenshot.format
                    });
